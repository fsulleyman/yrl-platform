...
export default async function AdminPage() {
  ...
  // 1. Unauthenticated state: render Supabase Auth login
  if (authResult.status === 'unauthenticated') {
    return <AdminLoginForm />;
  }

  // 2. Authenticated user without approved administrative role
  if (authResult.status === 'unauthorized_role') {
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[4px] border border-red-200 shadow-md p-8 text-center">
          <div className="w-12 h-12 rounded-full bg-red-100 text-red-700 flex items-center justify-center mx-auto mb-4">
            <ShieldAlert className="w-6 h-6" />
          </div>
          <h1 className="text-lg font-bold text-[#0E1E3B] mb-2">Access Restricted</h1>
          <p className="text-xs text-slate-600 leading-relaxed mb-4">
            You are signed in as <strong className="font-mono">{authResult.user.email}</strong>, but your account
            does not have an assigned administrative role (Super Admin, National Reviewer, or Regional Coordinator).
          </p>
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-[2px] text-xs text-amber-900 mb-6 text-left">
            ...
            Please contact the Super Admin to configure your role permissions.
          </div>
